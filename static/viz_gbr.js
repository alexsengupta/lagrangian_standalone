// GBR-focused Mapbox visualization with bathymetry, grid,
...
function vizMap(simData) {
  ...
  map.on('load', () => {
    ...
    // Grid
    const gridFeats = [];
    for (let x = boundsSW[0]; x <= boundsNE[0]; x += 2) {
      gridFeats.push({
        type: 'Feature',
        geometry: { type: 'LineString', coordinates: [[x, boundsSW[1]], [x, boundsNE[1]]] }
      });
    }
    for (let y = boundsSW[1]; y <= boundsNE[1]; y += 2) {
      gridFeats.push({
        type: 'Feature',
    ...
    // Reefs
    map.addSource('reefs', { type: 'geojson', data: '/lagrangian_6Aug/static/reef_locs_aus.geojson' });
    map.addLayer({
        id: 'reefs',
        type: 'circle',
        source: 'reefs',
...
function emptyGeo() {
  ...
  saveBtn.onclick = () => {
      ...
      a.href = url;
      a.download = 'simulation_data_gbr.csv'; // Set the filename to .csv for GBR
